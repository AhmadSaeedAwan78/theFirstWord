...
<div class="container">
	<div class="row">
		<div class="col-md-8 col-md-offset-2">
			<div class="panel panel-default">
				<div class="panel-heading"><strong>{{ __('Two Factor Authentication') }}</strong></div>
	...
	<br/>
		...
		<strong>1. {{ __('Scan this barcode with your Google Authenticator App') }}:</strong><br/>
		<img src="data:image/png;base64,{{$data['google2fa_image'] }}" alt="" />
		<br/><br/>
		<strong>2.{{ __('Enter the pin the code to Enable 2FA') }}</strong><br/><br/>
		<form class="form-horizontal" method="POST" action="{{ route('enable2fa') }}">
		{{ csrf_field() }}
		<div class="form-group{{ $errors->has('verify-code') ? ' has-error' : '' }}">
		<label for="verify-code" class="col-md-4 control-label">{{ __('Authenticator Code') }}</label>
		<div class="col-md-6">
	...
	@elseif($data['user']->passwordSecurity->google2fa_enable)
	<div class="alert alert-success">
	{{ __('2FA is Currently') }} <strong>{{ __('Enabled') }}</strong> {{ __('for your account') }}.
	</div>
	...
	<label for="change-password" class="col-md-4 control-label">{{ __('Current Password') }}</label>
	<div class="col-md-6">
